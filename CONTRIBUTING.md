...
- Changes under `samples/` are detected and validated to the repository's supported build-readiness level.
- Pull requests from forks run without repository credentials. The required `trusted` check is expected to remain blocked until a maintainer promotes the exact commit to a same-repository branch for trusted validation.
