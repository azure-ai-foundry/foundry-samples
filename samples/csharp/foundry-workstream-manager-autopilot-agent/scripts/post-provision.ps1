...
# -----------------------------------------------------------------------------
# One-time digital worker setup: publishing the digital worker, creating the
# blueprint SP OAuth2 grants, and adding the current user as blueprint owner only
# need to happen on the FIRST successful provision. They are idempotent, but they
# make unnecessary API calls on every code-change re-provision, so we persist a
# marker in the azd environment and skip them once completed.
#
