...
A long-running deep-research agent for the Foundry hosted-agent platform
(invocations protocol). It runs a multi-phase research plan where each phase
issues several streaming LLM sub-calls, and it survives container restarts,
OOM kills, and redeployments without losing its place.
...
layer subscribes to the same stream by id (see ``main.py``). On crash
...
- ``FOUNDRY_PROJECT_ENDPOINT`` — Azure AI Foundry project endpoint
  (auto-injected when hosted). When unset, the agent runs in OFFLINE demo
  mode with deterministic synthetic token streams — no credentials needed.
- ``AZURE_AI_MODEL_DEPLOYMENT_NAME`` — model deployment name
  (default: ``gpt-5.4-mini``).
- ``NUM_PHASES`` — number of research phases (default: 6).
- ``CALLS_PER_PHASE`` — sub-calls per phase (default: 3, max 4).
- ``TARGET_OUTPUT_TOKENS`` — soft cap for per-subcall LLM output
  (default: 800).
- ``INTRA_PHASE_COOLDOWN_SEC`` — wait between subcalls in a phase
  (default: 3).
- ``INTER_PHASE_COOLDOWN_SEC`` — wait between phases (default: 6).
"""
...
_model = os.environ.get("AZURE_AI_MODEL_DEPLOYMENT_NAME", "gpt-5.4-mini")

# Offline demo mode: with no Foundry endpoint we stream deterministic synthetic
# text instead of calling the model, so the full resilient / streaming / steering
# control flow runs with no Azure credentials.
OFFLINE_MODE = not _endpoint
...
# Defaults are modest so a local run finishes in a few minutes. Raise them
# (via env in azure.yaml or your shell) to turn this into a genuine long-runner
# that exercises multi-restart recovery.
NUM_PHASES = max(1, int(os.environ.get("NUM_PHASES", "6")))
CALLS_PER_PHASE = max(1, min(len(_SUB_CALL_ROLES), int(os.environ.get("CALLS_PER_PHASE", "3"))))
TARGET_OUTPUT_TOKENS = int(os.environ.get("TARGET_OUTPUT_TOKENS", "800"))
INTRA_PHASE_COOLDOWN_SEC = float(os.environ.get("INTRA_PHASE_COOLDOWN_SEC", "3"))
INTER_PHASE_COOLDOWN_SEC = float(os.environ.get("INTER_PHASE_COOLDOWN_SEC", "6"))
...
async def _stream_llm(emit: EmitFn, *, instructions: str, user_input: str) -> str:
    ...
    In OFFLINE demo mode (no ``FOUNDRY_PROJECT_ENDPOINT``) this streams
    deterministic synthetic tokens so the resilient / streaming / steering
    control flow runs end to end with no Azure credentials.
    """
    if OFFLINE_MODE:
        return await _stream_offline(emit, user_input=user_input)
...
async def _stream_offline(emit: EmitFn, *, user_input: str) -> str:
    """Deterministic token stream for offline demo mode."""
    words = (
        f"Analyzing '{user_input[:60]}'. This synthetic passage stands in for a "
        "streamed model response so the resilient-task, streaming, and steering "
        "behavior can be exercised without any Azure credentials. Replace with a "
        "real model call by setting FOUNDRY_PROJECT_ENDPOINT."
    ).split()
    full_text = ""
    for word in words:
        token = word + " "
