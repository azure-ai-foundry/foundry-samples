...
def _session_id(request: Request) -> str:
    """Resolve the session id on any route.

    On GET/cancel routes the framework does not always populate
    ``request.state.session_id`` (it is derived from the platform session when
    hosted, or from an explicit ``?agent_session_id=`` query param locally), so
...
async def handle_invoke(request: Request) -> Response:
    ...
    await deep_research.start(
        ...
        input={
            ...
            # Persist the platform call_id in the task input so it is restored
            # on recovery (the framework re-invokes with the same input).
...
async def handle_get(request: Request) -> Response:
    ...
    session_id: str = _session_id(request)
    if not session_id:
        return JSONResponse(
    ...
    task_id = f"research-{session_id}"
    # Task.get + TaskSnapshot removed. Use the provider directly for read-only
    # inspection (returns TaskInfo).
...
async def handle_cancel(request: Request) -> Response:
    ...
    session_id: str = _session_id(request)
    if not session_id:
        return JSONResponse({"status": "not_found", "message": "Provide ?agent_session_id=<id>."})
    task_id = f"research-{session_id}"
