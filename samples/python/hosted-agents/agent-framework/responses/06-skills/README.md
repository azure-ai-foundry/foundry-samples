...
The script writes the PDF to `$HOME/generated-travel-guides` by default and returns both the filesystem path and a `sandbox:` URI to the agent. Clients that support sandbox file retrieval can use that URI to fetch the generated PDF. For production scenarios that need durable links, update the skill script to upload the PDF to storage and return a shareable URL.
